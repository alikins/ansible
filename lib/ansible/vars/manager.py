    ...
    def get_vars(self, play=None, host=None, task=None, include_hostvars=True, include_delegate_to=True, use_cache=True):
        ...
        if host:
            ...
            def _plugins_inventory(entities):
                ...
                for inventory_dir in self._inventory._sources:
                    ...
                    for plugin in vars_loader.all():
                        data = combine_vars(data, _get_plugin_vars(plugin, inventory_dir, entities), name_b='inventory_plugin_dir_%s' % plugin, scope_info=inventory_dir)
            ...
            def _plugins_play(entities):
                ...
                data = {}
                for plugin in vars_loader.all():
                    data = combine_vars(data, _get_plugin_vars(plugin, basedir, entities), name_b='plugins_play_%s' % plugin, scope_info=plugin._original_path)
                return data
