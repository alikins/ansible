...
class PlaybookContextLoggingFilter(object):
    ...
    def filter(self, record):
        ...
        if self.playbook_context.playbook:
            record.playbook = self.playbook_context.playbook.basename
        ...
        if self.playbook_context.play_uuid:
            record.playbook_uuid = self.playbook_context.play_uuid
...
class TaskResultRepr(object):
    ...
    def str_impl(self):
        ...
        parts.append("    task(%s): %s" % (type(self.task), self.task))
...
# TODO: This represent a single state, ideally the current state. The
#       next step would be to track a stack of context and push/pop to it
#       as we progress.
# TODO: track current state and a history
class PlaybookContext(object):
    ...
    def logger_name(self, base_logger_name=None):
        ...
        if self.playbook:
            logger_name += '.%s' % self.playbook.basename
        if self.play:
...
class CallbackModule(CallbackBase):
    ...
    def _multiline_v2_on_any(self, *args, **kwargs):
        for arg in args:
    ...
    def v2_on_any(self, *args, **kwargs):
        ...
        for k, v in kwargs.items():
            kwargs_repr += ', %s=%s' % (k, v)
        #the_args = '%s, %s' % (args_repr, kwargs_repr)
        #self.logger.debug('args_repr=%s kvargs=%s' % (args_repr, kwargs_repr))
        #self.logger.debug('v2_on_any(%s)', the_args)
    ...
    def v2_playbook_on_start(self, playbook):
        # self.playbook = playbook
        # TODO: self.context_stack.push(playbook)
    ...
    def v2_playbook_on_play_start(self, play):
        # self.play = play
        # TODO: self.context_stack.push(play)
    ...
    def v2_playbook_on_task_start(self, task, is_conditional):
        ...
        # TODO: self.context_stack.push(task=...)
