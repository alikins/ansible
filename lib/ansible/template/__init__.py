                        ...
                        if convert_data and not self._no_type_regex.match(variable):
                            ...
                            if (result.startswith("{") and not result.startswith(self.environment.variable_start_string)) or \
                                ...
                                if eval_results[1] is None:
                                    ...
                                    if unsafe:
                                        self.log.debug('(UNSAFE) %s (type=%s) was eval() to be unsafe, will be wrapped as unsafe',
                                                       repr(variable), type(variable))
    ...
    def _contains_vars(self, data):
        ...
        if isinstance(data, string_types):
            for marker in (self.environment.block_start_string, self.environment.variable_start_string, self.environment.comment_start_string):
                if marker in data:
                    self.log.debug('%s contains_vars: %s', repr(data), repr(marker))
                    return True
        return False
